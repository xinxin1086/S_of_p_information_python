...
- created_at: 创建时间
- updated_at: 更新时间
- author_user_id: 发布者用户ID（外键 → user_info.id，用户注销后设为 NULL）
- author_admin_id: 发布者管理员ID（外键 → admin_info.id，管理员注销后设为 NULL）
- author_type: 发布者类型（enum: USER/ORG_USER/ADMIN）
- author_display: 发布者显示名（用户注销后改为"用户已注销"）
...
**科普文章批量更新作者显示信息**
- POST `/api/admin/science/update-author-displays`
- 需要认证: 是
- 需要权限: 超级管理员 (SUPER_ADMIN)
- 返回: 批量更新结果（处理用户注销后的匿名化）
...
#### 权限对比表
| 功能模块 | 具体接口 | 超级管理员 | 普通管理员 |
|---------|---------|----------|----------|
| **基础权限** | | | |
| 管理员登录 | `/api/admin/login` | ✅ | ✅ |
| 个人信息查询 | `/api/admin/info` | ✅ | ✅ |
| 权限查询 | `/api/admin/permissions` | ✅ | ✅ |
| **用户管理** | | | |
| 用户增删改查 | `/api/admin/operate` (user_info) | ✅ | ✅ |
| **内容管理** | | | |
| 科普文章管理 | `/api/admin/operate` (science_articles) | ✅ | ✅ |
| 活动管理 | `/api/admin/operate` (activities) | ✅ | ✅ |
| 公告管理 | `/api/admin/operate` (notice) | ✅ | ✅ |
| **高级权限** | | | |
| 管理员管理 | `/api/admin/operate` (admin_info) | ✅ | ❌ |
| 批量更新用户显示 | `/api/admin/activity/update-user-displays` | ✅ | ❌ |
| 批量更新文章作者 | `/api/admin/science/update-author-displays` | ✅ | ❌ |

#### 权限控制机制
- **装饰器控制**: 使用 `@permission_required` 装饰器保护接口
- **表级权限**: 通过 `check_table_permission()` 函数控制对特定表的操作
- **操作级权限**: 控制增删改查等具体操作权限
- **动态验证**: 在 `execute_operation` 中动态验证权限
